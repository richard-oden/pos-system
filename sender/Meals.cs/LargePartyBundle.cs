...
namespace PosSender
{
    public class LargePartyBundle : IMeal, IOrderItem
    {
        ...
        public bool IsComplete => Contents.Count == 8;

        public LargePartyBundle(List<Item> contents = null)
        {
            ...
            {
                throw new InvalidMealException($"A {Name} can only contain four party sides and four party entrees.");
            }
        }
        ...
        private bool validateContents(List<Item> contents)
        {
            return contents.Count <= 2 &&
                contents.Where(i => i.Size == Size.PartySide).ToArray().Length <= 4 &&
                contents.Where(i => i.Size == Size.PartyEntree).ToArray().Length <= 4 &&
