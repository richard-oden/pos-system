...
namespace PosSender
{
    public class Bowl : IMeal, IOrderItem
    {
        public string Name {get; private set;} = "bowl";
        public decimal Price => Contents.Sum(i => i.Price);
        public List<Item> Contents {get; private set;}
        public bool IsComplete => Contents.Count == 2;

        public Bowl(List<Item> contents = null)
        {
            ...
            {
                throw new InvalidMealException($"A {Name} can only contain a 16oz side and 8oz entree.");
            }
        }
        ...
        private bool validateContents(List<Item> contents)
        {
            return contents.Count <= 2 &&
                ...
                contents.Where(i => i.Size == Size.Entree8oz).ToArray().Length <= 1 &&
