...
namespace PosSender
{
    public class Plate : IMeal, IOrderItem
    {
        public string Name {get; private set;} = "plate";
        public decimal Price => Contents.Sum(i => i.Price);
        public List<Item> Contents {get; private set;}
        public bool IsComplete => Contents.Count == 3;

        public Plate(List<Item> contents = null)
        {
            ...
            {
                throw new InvalidMealException($"A {Name} can only contain a 16oz side and two 8oz entrees.");
            }
        }
        ...
        private bool validateContents(List<Item> contents)
        {
            return contents.Count <= 2 &&
                ...
                contents.Where(i => i.Size == Size.Entree8oz).ToArray().Length <= 2 &&
